...
cmd('let g:blamer_enabled=1')     -- enable 'gitlens'-feature
...
-- disables auto-comment continuations for .py files (some filetypers have their format-options overwritten by ftplugins)
...
action_on_write('~/.config/dwmblocks/config.h', '!cd ~/.config/dwmblocks/; sudo make install; kill $(pidof -s dwmblocks) >/dev/null; dwmblocks &')
action_on_write('~/.xresources', 'autocmd BufWritePost ~/.xresources !xrdb -DPYWAL_="<$HOME/.cache/wal/colors.Xresources>" -merge ~/.xresources')
